...
description: >-
  ...
  verbatim — spec match plus behavioral defects: local correctness, removed
  behavior/regressions, contract and cross-file breaks, concurrency/reliability, language
  pitfalls, devex breakage, test correctness. The angle rubric and Finding Schema are built
  in; it never edits, and it never substitutes its own model for Codex. Security is out of
  scope.
model: opus
color: cyan
