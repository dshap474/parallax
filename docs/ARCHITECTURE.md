...
4. **Delegate the build with its review round.** Fable hands one Opus worker (`claude-worker`) the final plan spec plus the reviewer persona names resolved from the config. The worker implements, self-verifies, **spawns the three Codex-xhigh review lanes itself** — nested read-only subagents, in parallel, each handed a neutral review brief (the correctness lane covers spec match, bugs, robustness, and failure paths; the cleanup lane covers reuse, simplification, and efficiency; the structural lane covers maximalist maintainability — sprawl, ownership, boundaries) — then triages every finding with its build context still hot: fix it or rebut it with evidence, never silently drop it. One round, hard cap; then it re-verifies.
...
6. **Fable gates the work.** Now — and only now — it reads the diff once, with fresh eyes: do the rebuttals hold, do the residuals matter, did the worker and the review lanes miss something? By this point the cheap mistakes are caught; Fable's judgment goes on what's left. It fixes nits inline, then re-runs the plan's verification commands. (Escape hatch: structural rework → a fresh spec back through the build.)
7. **Docs + commit.** Fable updates `.project/` documentation itself — from the plan it authored and the diff it read at the gate — then a **local commit only** — never a push, PR, or publish.

The pipeline has exactly **two top-level subagent spawns** (1 plan-critic + 1 builder); the builder spawns the 3 review lanes nested inside its own context, so findings traffic never touches Fable's window. Fable writes `.project/` docs itself, from context it already holds (the plan it authored, the diff it read at the gate), so no bulk read enters its window.
...
- The build worker spawns only the reviewer personas the orchestrator names in its dispatch — never the plan-critic, planners, or other workers. (The frontmatter `Agent(...)` allowlist documents this; the binding rule lives in the persona prose.)
...
- Every dev run ends with Fable updating `.project/` docs and a local commit — never a push, PR, or publish step.
