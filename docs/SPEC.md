...
- Standalone `/plx:plan` resolves exactly one engine per critic dimension and launches both unless the current message explicitly substitutes or skips one. Shipped Codex bindings use `gpt-5.6-sol` at `xhigh`. Each required dimension gets one same-binding retry; a missing result yields `[RED-TEAM INCOMPLETE]`. A fundamental redesign gets one final two-critic pass, never an open loop. `/plx:dev` retains proportional plan sizing.
...
1. **Brief headers** — every brief file opens with the section header its rubric expects: `## Review brief` (reviewer-*), `## Task brief` (planner), `## Draft plan` (plan-critic-*), `## Spec` (worker — both build specs and fix briefs). Every plan-critic consumer separates `Original request`, `Confirmed decisions`, and `Candidate plan`; confirmed decisions override conflicting original wording.
