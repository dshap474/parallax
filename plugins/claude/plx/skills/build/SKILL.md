...
`plx-engine` path as `<plx-engine>`. Create `<tmp>` with
...
Run `plx-preflight --repo <repo> --require-claude --require-grok`
before mutation. If either engine is unavailable, record an aborted run and stop.
For Grok preflight and review calls, disable the Bash sandbox (`dangerouslyDisableSandbox: true`); keep Grok's kernel sandbox active.
