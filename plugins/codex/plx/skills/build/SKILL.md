---
name: build
description: Delegate an accepted spec to one fresh Codex gpt-5.6-sol Medium implementation agent, review the result with three read-only Grok 4.6 XHigh lanes, fix confirmed findings in the host, run the complete relevant verification suite, and report.
...
accepted implementation spec. Delegate that spec to one fresh Codex implementation agent,
run an independent Grok review, fix confirmed findings yourself, run the complete relevant
verification suite, and report the result.

This standalone workflow is separate from `$plx:dev`. It always launches exactly one fresh
same-host writer lane: Codex `gpt-5.6-sol` at `medium`. The active host remains the
orchestrator, reviewer, targeted fixer, and final verifier. Resolve `<plugin-root>` from
this loaded `SKILL.md` path by removing `/skills/build/SKILL.md`. Engine execution is
allowed only through packaged `<plugin-root>/bin/` tools.
...
- Write `Implementation: 1× fresh codex (gpt-5.6-sol medium) · review: 3×1 (grok-4.6 xhigh) · fixes: host · verification: full relevant suite`
...
- Run `<plugin-root>/bin/plx-preflight --repo <repo> --require-codex --require-grok`
  before mutation. If either required engine is unavailable, record an aborted run and
...
Launch exactly one fresh Codex implementation agent:
...
```text
Built: <what changed>
Implementation: <fresh Codex writer result and summary>
...
- Launch exactly one fresh Codex writer at `gpt-5.6-sol` `medium`; never launch a second
