---
name: build
description: Implement an accepted spec directly in the Codex host, review the result with three read-only Grok lanes, fix confirmed findings, run the complete relevant verification suite, and report.
...
Clean up with `<plugin-root>/bin/plx-clean-temp <tmp>`. This skill does not commit or
publish.
