...
```
  ...
  `<plugin-root>/bin/plx-eval finish` leaves the run incomplete.
...
Then clean up with `<plugin-root>/bin/plx-clean-temp <tmp>`. Interruption before this
step leaves the run `incomplete`.
End with:
